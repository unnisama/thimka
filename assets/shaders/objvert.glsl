...
{
    TexCoord = texCoord;
    //vnormal = normalize((umodel[gl_InstanceID] * vec4(Normal, 1.0)).xyz);
    mat4 curumodel = umodel[gl_InstanceID];

    vec4 position = curumodel * vec4(aPos, 1.0);
    //position.xz += viewpos.xz;
    position.y = perlin(time + (curumodel[3]).xz*scale, 1.0);
